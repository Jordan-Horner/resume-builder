...
export const VIEW_KEY = "resume-builder.job-view.v6";
export const EMPTY_VIEW: ViewFilters = { roles: [], workModes: [], country: "", locations: [], employmentTypes: [], minimumPay: null, currency: "USD", period: "year", includeUnknownPay: true, includeUnknownMode: true, includeUnmatchedLocation: false, includeClearanceJobs: true };
...
export function restoreView(defaults: ViewFilters): { filters: JobFilters; previous: ViewFilters; updated?: boolean } {
  ...
  const raw = localStorage.getItem(VIEW_KEY) || localStorage.getItem("resume-builder.job-view.v5") || localStorage.getItem("resume-builder.job-view.v4") || localStorage.getItem("resume-builder.job-view.v3") || localStorage.getItem("resume-builder.job-view.v2");
  if (!raw) {
  ...
  if (![candidate.roles, candidate.locations, candidate.workModes, candidate.employmentTypes].every((items) => Array.isArray(items) && items.every((item) => typeof item === "string")) || typeof candidate.country !== "string" || !/^[A-Z]{3}$/.test(candidate.currency) || !["year", "hour"].includes(candidate.period) || (candidate.includeClearanceJobs !== undefined && typeof candidate.includeClearanceJobs !== "boolean") || ![0, 1, 3, 7, 14, 30].includes(saved.filters.dateDays) || typeof saved.filters.search !== "string" || (candidate.minimumPay !== null && (!Number.isFinite(candidate.minimumPay) || candidate.minimumPay < 0))) throw new Error("Saved filters are invalid. Your job preferences have been restored.");
