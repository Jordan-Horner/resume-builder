...
- Upload-ready PDFs: `exports/resumes/<baselines|tailored>/<resume-slug>/<candidate-name>-Resume.pdf`
...
For version 7 or later plans, the selected named content template is authoritative for
section inclusion and order. Use the dedicated
[resume template contract](resume-template-contract.md). The following order is
legacy guidance for earlier plans, not a substitute for a template choice.
